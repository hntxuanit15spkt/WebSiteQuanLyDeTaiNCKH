...
public class ThongTinGiangVien extends HttpServlet {
	...
	if (username != null) {
		...
		while (rs.next()) {
		    ThongTinNguoiDung quanlygv = new ThongTinNguoiDung(rs.getString("TenDangNhap"),
			    rs.getString("MatKhau"), rs.getString("HoTen"), rs.getString("SoDienThoai"),
			    rs.getDate("NgaySinh"), rs.getBoolean("TrangThaiNguoiDung"), rs.getInt("MaLoaiNguoiDung"));
	...
	try {
	    c = connect.DBConnect.getConnection();
	    String sql = "delete from taikhoan, nguoidung using taikhoan, nguoidung where taikhoan.manguoidung = nguoidung.manguoidung and TenDangNhap = "
		    + "'" + tendangnhap + "'";
