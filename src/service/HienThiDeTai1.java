 ...
 * 
 * @WebServlet("/HienThiDeTai") public class HienThiDeTai1 extends HttpServlet {
 ...
 * session.getAttribute("username"); if (username != null) { List<DanhSachDeTai>
 * lstDeTai = new ArrayList<DanhSachDeTai>(); Connection c = null; try { c =
 ...
 * " or detai.MaGiangVienHuongDan = " + "'" + username + "')"; Statement stmt =
 * (Statement) c.createStatement(); ResultSet rs = stmt.executeQuery(sql); while
 * (rs.next()) { DanhSachDeTai dt = new DanhSachDeTai(rs.getString("HoTen"),
